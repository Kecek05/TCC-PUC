		...
		public void ScanObjects(ScanSettings settings, SheetAsset sheetAsset)
		{
			try
			{
				...
				foreach (var guid in guids)
				{
					...
					if (settings.IsPathExcluded(path))
					{
						continue;
					}
					...
					if (settings.IsFullTypeNameExcluded(assetType))
					{
						continue;
					}
					...
					foreach (var subAsset in subAssets)
					{
						...
						if (sheetAsset == SheetAsset.Prefab)
						{
							if (subAsset is Component)
							{
								...
								if (!settings.RootPrefabsOnly || isRootPrefab)
								{
									...
									if (settings.IsFullTypeNameExcluded(subAssetType))
									{
										continue;
									}
						...
						else
						{
							if (AssetDatabase.IsSubAsset(subAsset))
							{
								...
								while (nextSubAssetType.IsValidUnityObjectSubclass() && !settings.IsFullTypeNameExcluded(nextSubAssetType))
								{
				...
				if (sheetAsset == SheetAsset.ScriptableObject && settings.Option == ScanOption.Assembly)
				{
					...
					foreach (var assembly in assemblies)
					{
						...
						(
							type => !settings.IsFullTypeNameExcluded(type) && type.IsSubclassOf(typeof(ScriptableObject))
								&& type.IsSerializable
